...
train_label = np.array([1] * len(pos_train_text) + [0] * len(neg_train_text))
...
pred = estimator.predict(test_feature.todense())


pickle.dump(estimator, open("RF.pkl", "wb"), True)
PRED = []
for each in pred:
    if each == 1:
        PRED.append('+1')
...
print('evaling ...')
print(score('result_rf', 'test.txt'))
